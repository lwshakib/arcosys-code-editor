...
const startWatching = (folderPath) => {
  ...
  const notifyChange = () => {
    ...
    debounceTimer = setTimeout(() => {
      win == null ? void 0 : win.webContents.send("fs:changed");
    }, 10);
  };
  watcher.on("add", notifyChange).on("change", notifyChange).on("unlink", notifyChange).on("addDir", notifyChange).on("unlinkDir", notifyChange);
};
...
function createWindow() {
  win = new BrowserWindow({
    ...
    icon: sp__default.join(process.env.VITE_PUBLIC, "electron-vite.svg"),
    webPreferences: {
