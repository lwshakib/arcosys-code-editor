...
const MemoizedBigTitle = memo(BigTitle);
const MemoizedTips = memo(Tips);
const MemoizedContextInfo = memo(ContextInfo);
const MemoizedExitSummary = memo(ExitSummary);
...
// Memoized message list to prevent re-rendering on every keystroke
...
export const AppUI = ({ sessionToken }: { sessionToken: string }) => {
  ...
  if (isExiting) {
    return (
      <Box flexDirection="column" paddingX={4} paddingY={1} minHeight={rows} width={columns}>
      <MemoizedBigTitle text="ARCOSYS" />
        <MemoizedTips />
